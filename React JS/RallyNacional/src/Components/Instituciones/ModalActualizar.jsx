...
function ModalActualizar(prop)
{
   // const {datos} = prop.row;
    ...
    const Actualizar = async (event) =>
    {
        ...
        const { response } = await fetchData(`institucion/editar/${prop.row.id}`, prop.setEstado, {
            method: "PUT",
            data: inst,
        });
                ...
                <form onSubmit={(event) => Actualizar(event)}>
                    <Modal.Body>
                        ...
                        <div className="form-group">
                            ...
                            <input type="text" className="form-control" id="abreviacion" name="abreviacion"  onChange={handleChange} defaultValue={prop.row.abreviacion} required />
