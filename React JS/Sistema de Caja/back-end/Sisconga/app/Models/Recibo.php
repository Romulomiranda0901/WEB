...
class Recibo extends Model
{
    public function listar_recibos($id_caja, $anyo_proceso)
    {
        try {
            ...
            $datos_cordoba = $this->getCurrencyData($id_caja, $anyo_proceso, 1);
            $datos_dolares =  $this->getCurrencyData($id_caja, $anyo_proceso, 2);

            $datos_cordoba = json_decode($datos_cordoba, true) ?: 0;
            $datos_dolares = json_decode($datos_dolares, true) ?: 0;
            ...
            return [
                'datos_dolares' => $datos_dolares[0]['monto_total'] ?? $datos_dolares,
                'datos_cordoba' => $datos_cordoba[0]['monto_total'] ?? $datos_cordoba,
                'recibos' => $recibos,
            ];
    ...
    function getCurrencyData($id_caja, $anyo_proceso, $id_tipo_moneda) {
        return DB::table('tesoreria.recibos_detalle as rd')
            ->join('tesoreria.recibos as r', 'rd.id_recibos', '=', 'r.id')
            ->join('general.clientes as c', 'r.id_cliente', '=', 'c.id')
            ->join('general.organicas_cajas as oc', 'r.id_caja', '=', 'oc.id')
            ->join('general.tipo_moneda as tm', 'r.id_tipo_moneda', '=', 'tm.id')
            ...
            ->where('r.anyo', '=', $anyo_proceso)
            ->where('r.finalizado', '=', 'NO')
            ->where('r.id_tipo_moneda', '=', $id_tipo_moneda)
            ->select(
                'r.anyo',
                'r.id_tipo_moneda',
                DB::raw('SUM(CASE WHEN ra.id IS NULL THEN r.monto ELSE 0 END) as monto_total')
            )
            ->groupBy('r.anyo', 'r.id_tipo_moneda')
            ->get();
    }
    ...
    public function ver_recibos($id_recibos)
    {
        try {
            ...
            $datos =  DB::table('tesoreria.recibos_detalle as rd')
                ...
                ->leftjoin('tipo_aranceles_carrera as tac', function($join) {
                    $join->on('tac.id_carrera_se','=','cas.id')
                        ->where('tac.id_tipopago', '=', 'rd.id_tipo_pagp');
                })

                ->leftJoin('tipo_pago_facultad as tpf', function($join) {
                    $join->on('tpf.id_facultad','=','ca.id_facultad')
                        ->where('tpf.id_tipopago', '=', 'rd.id_tipo_pagp');
                })
                ->join('general.organicas_cajas as oc', 'r.id_caja', '=', 'oc.id')
                ...
                ->select(
                    ...
                    'tac.monto as pago_articulo_carrera',
                    'tpf.monto as pago_articulo_facultad'
                )
                ->groupBy(
                    'r.id',
                    'r.numero_recibo',
